...
export function ShareButtons({ recipe }: ShareButtonsProps) {
  ...
  const shareText = `Check out this amazing ${recipe.name} recipe! It takes only ${recipe.prepTime + recipe.cookTime} minutes to make and serves ${recipe.servings} people.`;
  const hashtags = ['recipe', 'cooking', 'food', ...recipe.tags].join(',');

  const shareUrls = {
    facebook: `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(currentUrl)}&quote=${encodeURIComponent(shareText)}`,
    twitter: `https://twitter.com/intent/tweet?text=${encodeURIComponent(shareText)}&url=${encodeURIComponent(currentUrl)}&hashtags=${encodeURIComponent(hashtags)}`,
    pinterest: `https://pinterest.com/pin/create/button/?url=${encodeURIComponent(currentUrl)}&media=${encodeURIComponent(recipe.image || '')}&description=${encodeURIComponent(shareText)}`,
    whatsapp: `https://wa.me/?text=${encodeURIComponent(`${shareText} ${currentUrl}`)}`,
    email: `mailto:?subject=${encodeURIComponent(`Recipe: ${recipe.name}`)}&body=${encodeURIComponent(`Hi! I found this delicious recipe for ${recipe.name} and thought you might like it:\n\n${recipe.description}\n\nPrep time: ${recipe.prepTime} minutes\nCook time: ${recipe.cookTime} minutes\nServings: ${recipe.servings}\nDifficulty: ${recipe.difficulty}\n\nIngredients:\n${recipe.ingredients.map(ing => `- ${ing}`).join('\n')}\n\nCheck it out here: ${currentUrl}`)}`
  };
    ...
    <div className="border-t border-gray-200 pt-6 mt-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-3 bg-sky-500 text-white rounded-lg hover:bg-sky-600 transition-colors text-sm font-medium"
        >
          <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-3 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors text-sm font-medium"
        >
          <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-3 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition-colors text-sm font-medium"
        >
